...
三大法人買賣超、借券**的個人 PWA。每個交易日**盤前一小時**（台北時間
08:00）由 GitHub Actions 自動抓取前一交易日收盤後的完整資料。

資料來源：股價、估值、外資持股、借券仍來自 [FinMind](https://finmindtrade.com)
開放資料。**上市股票**的三大法人買賣超、融資融券改成直接讀證交所官方
OpenAPI（`MI_MARGN`、`T86`），單位/欄位錯誤風險比第三方轉手資料低；官方
來源當天若抓不到（連線失敗、回應格式跟預期不同、或該股票不在當天名單
內），會自動退回原本的 FinMind 抓法，兩者不會互相影響其他欄位。**上櫃股票**
的這兩項目前仍用 FinMind（櫃買中心也有對應官方 API，但尚未串接、需要另外
verify 格式後再接）。想確認某一天實際用了哪個來源，看 GitHub Actions 執行
紀錄裡 `[official]` / `[finmind-fallback]` 開頭的那幾行就知道。
